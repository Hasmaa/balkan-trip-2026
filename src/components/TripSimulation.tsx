...
export function TripSimulation({ selectedDay, start, onStep }: { selectedDay: number; start: string; onStep: (moment: SimulationMoment) => void }) {
  ...
  return <section className="trip-simulation" aria-label="Road trip simulation"><div className="simulation-heading"><div><p className="eyebrow">Play the roadtrip</p><h2>Follow the day as it unfolds.</h2></div><span className="simulation-day">{formatTripDate(dateForDay(start, moment.day))} · {moment.time}</span></div><div className="simulation-scene"><div className={`simulation-icon ${moment.kind}`}><CarFront aria-hidden="true"/></div><div><p className="eyebrow">{countries[moment.country].flag} {countries[moment.country].name} · {moment.kind}</p><h3>{moment.title}</h3><p>{moment.detail}</p><a className="simulation-preview" href={mapsSearchUrl(latLng(moment.coordinates))} target="_blank" rel="noopener noreferrer"><MapPin size={13} aria-hidden="true"/> Preview this stop on Google Maps</a></div></div><div className="simulation-track" aria-label={`${progress}% through itinerary`}><span style={{width:`${progress}%`}}/></div><div className="simulation-controls"><button onClick={()=>step(-1)} disabled={index===0} aria-label="Previous moment"><SkipBack/></button><button className="play" onClick={()=>setPlaying(p=>!p)}>{playing?<><Pause/>Pause</>:<><Play/>Play trip</>}</button><button onClick={()=>step(1)} disabled={index===total-1} aria-label="Next moment"><SkipForward/></button><button onClick={()=>{setPlaying(false);setIndex(0)}} aria-label="Restart trip"><RotateCcw/></button><button className={speed===2?'selected':''} onClick={()=>setSpeed(s=>s===1?2:1)}>{speed}×</button></div></section>;
}
