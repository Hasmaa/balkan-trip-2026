...
export function OverviewMap({ days, resources, selected, focus, start, onSelectDay, onOpen }: { days: TripDay[]; resources: Resource[]; selected: number; focus?: Focus; start: string; onSelectDay: (n: number) => void; onOpen: (r: Resource) => void }) {
  ...
  if (failed) return <div className="map-fallback"><b>Map unavailable</b><p>The planned route is still available:</p><ol>{days.map(d => <li key={d.day}><button onClick={() => onSelectDay(d.day)}>{formatTripDate(dateForDay(start, d.day))} — Day {d.day}: {d.title}</button></li>)}</ol></div>;
